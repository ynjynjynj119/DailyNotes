...
class apiController extends appController
{
     ...
     * @author EasyChen
     */
	public function something_add()
	{
		$content = z(t(v('text')));
		if( !not_empty($content) ) return self::send_error(  LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'TEXT' ) );
					
		if( !$tid = add_something( $content ))
		{
			return self::send_error( LR_API_DB_ERROR , __('API_MESSAGE_DATABASE_ERROR') . db_error() );
		}

		$tinfo = get_something_info_by_id( $tid );		
	...
	{
		return $this->todo_set_value( 'is_public' , 0 ,9);
	}
	...
	{
		$this->todo_set_value( 'is_star' , 0 );
		return $this->todo_set_value( 'status' , 3, 1 );
	}
	...
	{
		return $this->todo_set_value( 'status' , 1, 2 );
	...
	* @ignore
	*/
	private function todo_set_value( $field , $value, $endcom=0 )
	{
		$tid = intval(v('tid'));
...
if($endcom==1){ //关闭1 打开2
	...
	$sql = "INSERT INTO `todo_history` ( `tid` , `uid` , `content` , `type` , `timeline` ) VALUES ( '" . intval($tid) . "' , '" . intval($_SESSION['uid']) . "' , '".__('TODO_END')."' , 1 , NOW() )";
...
if($endcom==2){
	...
	$sql = "DELETE FROM `todo_history` WHERE `tid` = '" . intval($tid) . "' and `uid` = '" . intval($_SESSION['uid']). "' and `content` = '".__('TODO_END')."' LIMIT 1";
	run_sql( $sql );
...
if($days==9){
	...
	$sql = "UPDATE `todo_user` SET `last_action_at` = DATE_ADD(NOW(),INTERVAL 1 DAY) WHERE `tid` = '" . intval( $tid ) . "' ";
	run_sql( $sql );
		...
		else
		{
			...
			if( is_array( $data ) )
			{
				...
				foreach( $data as $hitem )
				{
					...
					if( $hitem['tid'] > $max ) $max = $hitem['tid'];
					if( $hitem['tid'] < $min ) $min = $hitem['tid'];
				}
			}
