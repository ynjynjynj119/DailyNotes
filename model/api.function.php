...
function add_something( $text )
{
	...
	$sql = "INSERT INTO `todo_day` ( `content` ,  `timeline` , `owner_uid` ) VALUES ( '" . s( $text ) . "'  , NOW() , '" . intval( $uid ) . "' ) ";
	run_sql( $sql );
...
function get_todo_info_by_id( $tid , $write_db = null )
{
	...
	$hdata = get_data( "SELECT * FROM `todo_history` WHERE `tid` = '" . intval($tid) . "' ORDER BY `timeline` LIMIT 100" , $write_db );
...
function get_something_info_by_id( $tid , $write_db = null )
{
	if( $write_db != null ) $write_db = db();
	
	if(!$tinfo = get_line( "SELECT *,`id` as `tid` FROM `todo_day` WHERE `id` = '" . intval($tid) . "' LIMIT 1" , $write_db )) return false;	
